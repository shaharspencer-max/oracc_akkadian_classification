...
genre_normalization_dictionary = {
    # Case normalization
    ...
    "Legal Transaction": "Legal Document",
    
    # Scholarly categories  
    "Scholarly": "Scholarly Letter",
    ...
    # Astrological/Astronomical categories
    "Astrological": "Astrological Report",
    "Astronomical": "Astronomical Document",
    "Astronomical Diary": "Astronomical Diary",
    "Astrological Report": "Astrological Report",
    
    # Lexical categories
    "Lexical": "Lexical Document", 
    "Lexical Text": "Lexical Document",
    
    # Keep others as-is but ensure they're properly formatted
    ...
    "Grant": "Grant", 
    "Medical": "Medical",
    "Royal Ritual": "Royal Ritual",
    "Prayer/Incantation": "Prayer/Incantation",
    "Uncertain": "Uncertain",
    "Eponym Chronicle": "Eponym Chronicle",
    "Hemerological": "Hemerological",
    "Eponym List": "Eponym List", 
...
def get_prediction_dataframe(path: str)->pd.DataFrame:
    ...
    print("Normalizing genre values...")
    original_genre_counts = predictions_dataframe['genre'].value_counts()
    print(f"Original unique genres: {len(original_genre_counts)}")
    
    # Apply genre normalization
    predictions_dataframe['genre'] = predictions_dataframe['genre'].map(
        lambda x: genre_normalization_dictionary.get(x, x) if pd.notna(x) else x
    )
    
    # Show results of normalization
    normalized_genre_counts = predictions_dataframe['genre'].value_counts()
    print(f"Normalized unique genres: {len(normalized_genre_counts)}")
    print("\nTop 10 normalized genres:")
    print(normalized_genre_counts.head(10))
